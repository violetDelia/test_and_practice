import java.awt.*;
import javax.swing.*;

public class AutoScore extends JFrame {
    public static void main(String[] args) {
        new AutoScore();
    }

    public AutoScore() {
        init();
        setSize(400, 400);
        setDefaultCloseOperation(EXIT_ON_CLOSE);
        setVisible(true);
    }

    java.awt.Button btnNew = new java.awt.Button();
    java.awt.Button btnJudge = new java.awt.Button();
    java.awt.Label lblA = new java.awt.Label();
    java.awt.Label lblB = new java.awt.Label();
    java.awt.Label lblOp = new java.awt.Label();
    java.awt.Label lbl5 = new java.awt.Label();
    java.awt.TextField txtAnswer = new java.awt.TextField();
    java.awt.List listDisp = new java.awt.List();

    public void init() {
        setLayout(null);
        setSize(400, 400);
        btnNew.setLabel("set");
        getContentPane().add(btnNew);
        btnNew.setBackground(java.awt.Color.lightGray);
        btnNew.setBounds(36, 96, 98, 26);
        btnJudge.setLabel("judge");
        getContentPane().add(btnJudge);
        btnJudge.setBackground(java.awt.Color.lightGray);
        btnJudge.setBounds(216, 96, 98, 26);
        lblA.setText("text");
        getContentPane().add(lblA);
        lblA.setFont(new Font("Diolog", Font.PLAIN, 24));
        lblA.setBounds(36, 24, 36, 36);
        lblB.setText("text");
        getContentPane().add(lblB);
        lblB.setFont(new Font("Diolog", Font.PLAIN, 24));
        lblB.setBounds(108, 24, 36, 36);
        lblOp.setText("text");
        getContentPane().add(lblOp);
        lblOp.setFont(new Font("Diolog", Font.PLAIN, 24));
        lblOp.setBounds(72, 24, 36, 36);
        lbl5.setText("text");
        getContentPane().add(lbl5);
        lbl5.setFont(new Font("Diolog", Font.PLAIN, 24));
        lbl5.setBounds(168, 24, 36, 36);
        getContentPane().add(txtAnswer);
        txtAnswer.setFont(new Font("Diolog", Font.PLAIN, 24));
        txtAnswer.setBounds(216, 24, 85, 42);
        getContentPane().add(listDisp);
        listDisp.setFont(new Font("Diolog", Font.PLAIN, 24));
        listDisp.setBounds(36, 144, 272, 106);

        SymAction ISymAction = new SymAction();
        btnNew.addActionListener(ISymAction);
        btnJudge.addActionListener(ISymAction);
    }

    class SymAction implements java.awt.event.ActionListener {
        public void actionPerformed(java.awt.event.ActionEvent event) {
            Object object = event.getSource();
            if (object == btnNew)
                btnNew_Actionperformed(event);
            else if (object == btnJudge)
                btnJudge_Actionperformed(event);
        }
    }

    int a = 0, b = 0;
    String op = "";
    double result = 0;

    void btnNew_Actionperformed(java.awt.event.ActionEvent event) {
        a = (int) (Math.random() * 9 + 1);
        b = (int) (Math.random() * 9 + 1);
        int c = (int) (Math.random() * 4);
        switch (c) {
            case 0:
                op = "+";
                result = a + b;
                break;
            case 1:
                op = "-";
                result = a - b;
                break;
            case 2:
                op = "*";
                result = a * b;
                break;
            case 3:
                op = "/";
                result = a / b;
                break;
        }
        lblA.setText("" + a);
        lblB.setText("" + b);
        lblOp.setText("" + op);
        lbl5.setText("=");
        txtAnswer.setText("");
    }

    void btnJudge_Actionperformed(java.awt.event.ActionEvent event) {
        String s = txtAnswer.getText();
        double d = Double.valueOf(s).doubleValue();
        String disp = "" + a + op + b + "=" + "" + (int)result+"   ";
        if (d == result)
            disp += "right";
        else
            disp += "false";
        listDisp.add(disp);
    }
}
